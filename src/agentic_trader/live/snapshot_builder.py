...
async def build_snapshot(
    *,
    ...
    """Fetch the execution series (``exec_tf``) + Daily for ATR, then build pivot sets.

    ``exec_tf`` is the single scan execution timeframe (TV code, default M5 "5"); its
    bars populate ``snapshot.m5_bars`` and ``atr_m5`` and drive the stack bias.
    """
    m5_result = await fetcher.fetch_bars(symbol, exec_tf, n_bars=50)
